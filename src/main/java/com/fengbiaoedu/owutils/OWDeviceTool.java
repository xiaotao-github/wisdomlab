...
public class OWDeviceTool {
    static Properties mProperties;

    static{
        mProperties = new Properties();

        InputStream is = OWDeviceTool.class.getClassLoader().getResourceAsStream("static/cn/devices_ch.properties");
        try {
            mProperties.load(is);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    // 是否是警报设备
    public static boolean owIsAlarmDevByType(String devType) {
    ...
    // 是否是多控设备
    public static boolean owIsMutiCtrlDevByType(String devType) {
    ...
    // 是不是绑定场景设备
    public static boolean owIsBindSceneByType(String devType) {
    ...
    // 是否是传感器设备
    public static boolean owIsSensorDevByType(String devType) {
    ...
    // 是不是  不可控设备?
    public static boolean owIsNoQuickCtrlDevByType(String devType) {
        if (
    ...
    // 获取设备名
    public static String owGetDevDefaultNameByType(String devType) {
        String C = null;
    ...
    static String owGetResString(String key) {
    ...
    //TODO 获取通过数据和类型
    public static boolean owGetSendCtrlStatusByByTypeAndData(String epData,
                                                           String devType, String epStatus, boolean isCtrl) {
        if (owIsAlarmDevByType(devType)) {
     ...
     * @return
     */
    public static String owGetDevDataText(String epType, String epData,
                                        String epStatus) {
        String first;
        String second;
        String third;
        ...
        // 可燃气阀门，类型值10
        if (owIsAlarmDevByType(epType)) {
            if (ConstUtil.DEV_TYPE_FROM_GW_GAS_VALVE.equals(epType)) {
                // alarm
                sb.append(epData.startsWith("1") ? owGetResString("device_baojing")
                        : owGetResString("device_daijing"));
                sb.append("\t");
                sb.append(epData.endsWith("0") ? owGetResString("device_guan")
                        : owGetResString("device_ka"));
            } else {
                sb.append(epData.startsWith("0") ? owGetResString("device_daijing")
                        : owGetResString("device_baojing"));
                sb.append("\t");
                sb.append("0".equals(epStatus) ? owGetResString("device_cafang")
                        : owGetResString("device_bufang"));
            }
        }
        ...
        // 类型值11
            ...
            sb.append("0".equals(epData) ? owGetResString("device_guan")
                    : owGetResString("device_kai"));
        }
        ...
        // 一位调光灯，类型值12
            ...
            sb.append(0 == epDataInt ? owGetResString("device_guan")
                    : 100 == epDataInt ? owGetResString("device_kai") : epDataInt
                    + "%");
        }
        // 移位计量插座，类型值15
        else if (ConstUtil.DEV_TYPE_FROM_GW_EMS.equals(epType)) {
            sb.append(epData.startsWith("0101") ? owGetResString("device_kai")
                    : epData.startsWith("0100") ? owGetResString("device_guan")
                    : owGetResString("device_exception"));
        }
        // 移动普通插座，类型值16
        else if (ConstUtil.DEV_TYPE_FROM_GW_DOCK.equals(epType)) {
            sb.append(epData.startsWith("0") ? owGetResString("device_guan")
                    : owGetResString("device_kai"));
        }
        // 温湿度检测器，类型值17
        ...
        // 类型值26
            ...
            sb.append("2".equals(epData) ? owGetResString("device_kai") : "3"
                    .equals(epData) ? owGetResString("device_guan") : "4"
                    .equals(epData) ? owGetResString("device_stop")
                    : owGetResString("device_exception"));
        }
        ...
        // 水阀，类型值28
            ...
            boolean normal = "11".equals(epData);
            boolean timer = epData.startsWith("2");
            sb.append(normal || timer ? owGetResString("device_kai")
                    : owGetResString("device_guan"));
        } else if (owIsBindSceneByType(epType)) {
            sb.append(" ");
        }
        // 类型值41
        ...
        // 车位检测器，类型值46
            ...
            if (epData.contains(",")) {
                ...
                sb.append(first)
                        .append("MM")
                        .append("\t")
                        .append("0".equals(second) ? owGetResString("device_park_no_obstacle")
                                : "1".equals(second) ? owGetResString("device_park_has_obstacle")
                                : owGetResString("device_exception"));
            }
        }
        ...
        // 或一位照明开关，类型值61
                ...
                || owIsMutiCtrlDevByType(epType)) {
            sb.append(epData.endsWith("0") ? owGetResString("device_guan")
                    : epData.endsWith("1") ? owGetResString("device_kai")
                    : owGetResString("device_exception"));
        }
        ...
        // 窗帘控制器或百叶窗，类型值65或66
            ...
            sb.append("3".equals(epData) ? owGetResString("device_guan") : "2"
                    .equals(epData) ? owGetResString("device_kai")
                    : owGetResString("device_stop"));
        }
        ...
        // 一代门锁或三代门锁，类型值67或69
            ...
            if (epData.length() >= 5) {
                ...
                sb.append("1".equals(first) ? owGetResString("device_jiesuo")
                        : "2".equals(first) ? owGetResString("device_shangsuo")
                        : "3".equals(first) ? owGetResString("device_jiesuoyanshi")
                        : owGetResString("device_exception"));
                sb.append(" ");

                sb.append("10".equals(second) ? owGetResString("device_yibaoxian")
                        : "11".equals(second) ? owGetResString("device_weibaoxian")
                        : owGetResString("device_exception"));
                sb.append(" ");

                sb.append("21".equals(third) ? owGetResString("device_suoyikai")
                        : "22".equals(third) ? owGetResString("device_suoyiguan")
                        : "23".equals(third) ? owGetResString("device_baojing")
                        : owGetResString("device_exception"));
            }
        }
        ...
        // 二代门锁，类型值68
            ...
            if (epData.length() >= 5) {
                ...
                sb.append("10".equals(second) ? owGetResString("device_yibaoxian")
                        : "11".equals(second) ? owGetResString("device_weibaoxian")
                        : "25".equals(second) ? owGetResString("device_qiangzhishangsuo")
                        : "26".equals(second) ? owGetResString("device_zidongshangsuo")
                        : "30".equals(second) ? owGetResString("device_mimajiesuo")
                        : "31".equals(second) ? owGetResString("device_niukou")
                        + "1"
                        + owGetResString("device_jiesuo")
                        : "32".equals(second) ? owGetResString("device_niukou")
                        + "2"
                        + owGetResString("device_jiesuo")
                        : "33".equals(second) ? owGetResString("device_niukou")
                        + "3"
                        + owGetResString("device_jiesuo")
                        : "34".equals(second) ? owGetResString("device_niukou")
                        + "4"
                        + owGetResString("device_jiesuo")
                        : owGetResString("device_exception"));
                sb.append(" ");

                sb.append("23".equals(third) ? owGetResString("device_ruqinbaojing")
                        : "24".equals(third) ? owGetResString("device_jiechubaojing")
                        : "29".equals(third) ? owGetResString("device_pohuaibaojing")
                        : owGetResString("device_exception"));
            }
        }
    ...
    public static String owGetDevCtrlDataByType(String devType, int ctrlType) {
        if (ctrlType == 0) {
            return owGetOpenCtrlData(devType);
        } else if (ctrlType == 1) {
            return owGetCloseCtrlData(devType);
        } else if (ctrlType == 2) {
            return owGetStopOrDelayCtrlData(devType);
        } else {
            //KLog.i("控制返空null");
            return null;
        }
    ...
    //开启设备的控制命令
    private static String owGetOpenCtrlData(String devType) {
        //KLog.i("开");
        if (ConstUtil.DEV_TYPE_FROM_GW_DOCK.equals(devType)
                ...
                || ConstUtil.DEV_TYPE_FROM_GW_CONTROL_1.equals(devType)
                || ConstUtil.DEV_TYPE_FROM_GW_LIGHT_1.equals(devType)
                || owIsMutiCtrlDevByType(devType)) {
            if (ConstUtil.DEV_TYPE_FROM_GW_DUAL_D_LIGHT.equals(devType)) {
                return "100";
            } else {
                return "1";
            }
        } else if (owIsAlarmDevByType(devType)) {
            return "1";
        } else if (ConstUtil.DEV_TYPE_FROM_GW_LIGHT.equals(devType)
    ...
    //关闭设备命令
    private static String owGetCloseCtrlData(String devType) {
        //	KLog.i("关");
        if (ConstUtil.DEV_TYPE_FROM_GW_DOCK.equals(devType)
                || ConstUtil.DEV_TYPE_FROM_GW_DOCK_1.equals(devType)
                || ConstUtil.DEV_TYPE_FROM_GW_BUTTON_1.equals(devType)
    ...
    //根据命令判断设备的开关状态
    public static String owGetCloseOrOpenByTypeAndEpData(String devType,String epData) {
        if (ConstUtil.DEV_TYPE_FROM_GW_DOCK.equals(devType)
                || ConstUtil.DEV_TYPE_FROM_GW_DOCK_1.equals(devType)
                || ConstUtil.DEV_TYPE_FROM_GW_BUTTON_1.equals(devType)
                || ConstUtil.DEV_TYPE_FROM_GW_CONTROL_1.equals(devType)
    ...
    private static String owGetStopOrDelayCtrlData(String devType) {
        //	KLog.i("停");
    ...
    public static void  owControlDevice(DeviceInfo deviceInfo,String ep) {
        ...
        // 是否可控设备
        if (OWDeviceTool.owIsNoQuickCtrlDevByType(devType))
            return;
        // 设备状态，开或关
        boolean isOpened = OWDeviceTool.owGetSendCtrlStatusByByTypeAndData(data,
        ...
        if (ConstUtil.DEV_TYPE_FROM_GW_TOUCH_4.equals(devType)
            ...
            if( ConstUtil.DEV_TYPE_FROM_GW_IR_CONTROL.equals(devType)){
                NetSDK.sendControlDevMsg(gwId, devID, ep, devType,OWDeviceTool.owGetDevCtrlDataByType(devType, ctrlType));
            }


        } else if (OWDeviceTool.owIsAlarmDevByType(devType)) {
            // 设备是警报感应设备
            if (ConstUtil.DEV_TYPE_FROM_GW_GAS_VALVE.equals(devType)||
                    ConstUtil.DEV_TYPE_FROM_GW_WARNING.equals(devType)
                    ) {
                ...
                NetSDK.sendControlDevMsg(gwId, devID, "14", devType,
                        OWDeviceTool.owGetDevCtrlDataByType(devType, ctrlType));
            }
        } else {
            //KLog.i("NetSDK.sendControlDevMsg");// LOG
    ...
    //TODO  获取设备的Icon
    public static int owGetIconDevByType(String Type) {
        int res = 0;
	...
	* */
    ...
    // called by MainActivity List long click
    public static void  owCustomControlDevice(DeviceInfo deviceInfo, String ep, Integer code) {
        String gwId = deviceInfo.getGwID();
        String devID = deviceInfo.getDevID();
        // String devType = deviceInfo.getType();
        String devType = deviceInfo.getDevEPInfo().getEpType();
